...
`33647802903`. The multi-architecture build and Docker Hub push succeeded, but its first attempt's
retention step failed with HTTP 403 while deleting old tag `0.6.0`. The owner replaced the GitHub
secret `DOCKERHUB_TOKEN` with a Docker Hub token that authorizes Read, Write, and Delete. Attempt 2
then passed image publication and retention, and an independent public tag dry run confirmed that
no releases exceed the newest-three policy. The superseded Docker Hub token may now be revoked after
the owner confirms it is not used by another integration. Do not expose either token.
...
then continue with the requested cleanup or isolated AWS recovery exercise.
